  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {sortedModels.map((model) => {
        ...
        return (
          <div key={model.id} className="bg-black border border-gray-800 rounded-lg p-4">
            ...
            <div className="flex items-center mb-2">
              {model.image ? (
                <img src={model.image} alt={updatedId} className="w-8 h-8 mr-2 rounded-lg" />
              ) : (
            ...
            <p className="text-xs text-gray-400">
              Created: {model.created ? new Date(model.created * 1000).toLocaleString() : 'N/A'}
            </p>
          </div>
        );
      })}
